...
export default class LogsList extends React.Component {
  ...
  async initLogsListener() {
    ...
    // let from = await gex.manager().web3contract.getPastEvents('SchainCreated', {
    let from = await gex.manager().web3contract.getPastEvents('SchainCreated', {
    ...
    for (let i = 0; i < from.length; i ++) {
      ...
      let date = moment.utc(retValue.lifetime * 1000).format("YYYY/MM/DD HH:mm:ss");
      console.log(retValue.lifetime);
      console.log(date);
      //
      let dateTo = moment.utc((parseInt(retValue.creationDate) +
          parseInt(retValue.lifetime)) * 1000).format("YYYY/MM/DD HH:mm:ss");

      let logMessage = {
        'sChainOwner': retValue.owner,
        'sChainName': sChainName,
        'sChainStorage': retValue.storageBytes,
        'sChainLifetime': dateTo,
        'sChainCreatedAt': date,
        'sChainNodeNumber': retValue.maxNodes,
        'sChainDeposit': retValue.deposit,
        'sChainCpu': retValue.cpu,
        'sChainTps': retValue.transactionThroughput,
      };

      logs.push({
          'logName': logName, 'logLevel': logLevel, 'logDate': date, 'logMessage': JSON.stringify(logMessage),
          }
      )
    }
    //
